...
public class ChatActivity extends AppCompatActivity implements View.OnClickListener, ItemTouchHelperConnector {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Glide.with(this).load(photoUrl).placeholder(ContextCompat.getDrawable(ChatActivity.this,R.drawable.pet_foot)).into(photoUser);
    ...
    private void SendMassage(final String massage) {
        ...
        FirebaseDatabase.getInstance().getReference("Users").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                FirebaseDatabase.getInstance().getReference("Chats").child(FirebaseAuth.getInstance().getCurrentUser().getUid())
                        .child(userId).child(massageId).setValue(map);

                FirebaseDatabase.getInstance().getReference("Chats").child(userId)
    ...
    private void PopUpWindow(final MassageModel massageModel, final Integer index) {
        ...
        AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(ChatActivity.this,R.style.AlertTheme);
        ...
        btn_ok.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                {
                    FirebaseDatabase.getInstance().getReference("Chats").child(FirebaseAuth.getInstance().getCurrentUser().getUid())
                            .child(userId).child(massageModel.getMassageId()).removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {
                            FirebaseDatabase.getInstance().getReference("Chats").child(userId)
                                    .child(FirebaseAuth.getInstance().getCurrentUser().getUid()).child(massageModel.getMassageId()).removeValue().addOnSuccessListener(new OnSuccessListener<Void>() {
                                @Override
                                public void onSuccess(Void aVoid) {
                                    Toast.makeText(ChatActivity.this,getResources().getString( R.string.removed), Toast.LENGTH_SHORT).show();
                                }
                            });
